...
var tracesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Printf("For traceIDs %+v\n", args)
		inputfile, _ := cmd.Flags().GetString("file")
		outputfile, _ := cmd.Flags().GetString("outfile")
		isJson, _ := cmd.Flags().GetBool("json")
		if isJson {
			filterTracesToJson(inputfile, outputfile, args)
		} else {
			filterTraces(inputfile, outputfile, args)
		}
	},
}
...
func filterTracesToJson(inputFile string, outputFile string, traceIds []string) {
...
func filterTraces(inputFile string, outputFile string, traceIds []string) {
	spans, _, _ := common.PopulateStructsFromFile(inputFile)
	idSet := make(map[string]bool)
	for _, elem := range traceIds {
		idSet[elem] = true
	}
